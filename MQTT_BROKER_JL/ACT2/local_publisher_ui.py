...
main_layout = tk.Tk()
main_layout.title("MQTT UI Publisher")
...
topic = tk.StringVar(value="telemetry/topic")  # topic
client_data = tk.StringVar(value="NOM-PRENOM")
...
def config_ui_interface():
    ...
    log_box = scrolledtext.ScrolledText(main_layout, width=75, height=35)
    log_box.pack()
...
def get_system_info():
    ...
    mac = uuid.getnode()
    msg = f"date={str(date)} et mac={str(mac)}"
    return msg
...
def publish_start_button():
    ...
    # === Boucle pour transmettre les télémetries===
    while True:
        msg = get_system_info()
        current_topic = topic.get()
        client.publish(current_topic, msg)
        log_message(f"message={current_topic}:{msg}")
        time.sleep(4)
